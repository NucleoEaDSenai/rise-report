...
st.title("📊 Relatório de Caracteres - Cursos Rise")

uploaded_file = st.file_uploader("📂 Faça upload do ZIP exportado do Rise", type=["zip"])

if uploaded_file:
    with tempfile.TemporaryDirectory() as tmpdir:
        zip_path = os.path.join(tmpdir, "curso.zip")
        ...
        if not index_path:
            st.error("❌ Nenhum index.html encontrado no ZIP enviado.")
        else:
            html = open(index_path, "r", encoding="utf-8", errors="ignore").read()
